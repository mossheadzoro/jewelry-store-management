  ...
  return (
    <div className="p-8 max-w-[1400px] mx-auto w-full">
      ...
      <div className="flex items-center justify-between mb-10">
            ...
            <button
              ...
              className={`px-5 py-1.5 rounded-full text-[13px] font-medium transition-all ${
                activeTier === t 
                  ...
                  : "bg-[#111] text-[#666] border border-[#222] hover:text-foreground"
              }`}
            >
        ...
        <button className="h-10 px-6 rounded-xl bg-[#D4A843] text-foreground font-semibold text-[13px] flex items-center gap-2 hover:bg-[#e6bc5a] transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-12">
        {/* ELITE */}
        <div className="bg-onyx-surface border border-[#D4A843]/30 rounded-xl p-6 relative overflow-hidden">
          ...
          <h3 className="text-[36px] font-serif text-foreground leading-none mb-1">{stats.elite.active}</h3>
        ...
        {/* VIP */}
        <div className="bg-onyx-surface border border-[#222] rounded-xl p-6 hover:border-[#D4A843]/30 transition-colors">
          <div className="flex justify-between items-start mb-4">
          ...
          <h3 className="text-[36px] font-serif text-foreground leading-none mb-1">{stats.vip.active}</h3>
        ...
        <div className="bg-onyx-surface border border-[#222] rounded-xl p-6 hover:border-[#D4A843]/30 transition-colors">
          ...
          <h3 className="text-[36px] font-serif text-foreground leading-none mb-1">{stats.highValue.active}</h3>
        ...
        <div className="bg-onyx-surface border border-[#222] rounded-xl p-6 hover:border-[#D4A843]/30 transition-colors">
          ...
          <h3 className="text-[36px] font-serif text-foreground leading-none mb-1">{stats.wholesale.active}</h3>
      ...
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-[28px] font-serif text-foreground">Client Roster</h2>
        <div className="flex items-center gap-3">
          {isFetching && <div className="w-4 h-4 border-2 border-[#D4A843] border-t-transparent rounded-full animate-spin" />}
          <button className="px-4 py-2 bg-onyx-surface border border-[#222] rounded-lg flex items-center gap-2 text-[13px] text-[#aaa] hover:text-foreground transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {isLoading ? (
          [1, 2, 3].map(i => (
            <div key={i} className="bg-onyx-surface border border-[#222] rounded-xl h-[320px] animate-pulse" />
          ))
        ) : customers.length === 0 ? (
          <div className="col-span-full py-20 text-center text-[#666]">No clients found for this tier.</div>
        ) : (
          customers.map(c => (
            <div key={c.id} className="bg-onyx-surface border border-[#222] rounded-xl p-6 relative flex flex-col hover:border-border transition-colors">
              {c.loyaltyTier === "ELITE" && (
                <div className="absolute top-0 right-0 w-16 h-16 overflow-hidden">
                  <div className="absolute top-4 -right-4 w-24 bg-[#D4A843] text-foreground text-[10px] font-bold text-center py-1 rotate-45 transform origin-center shadow-lg">
                    ELITE
                  </div>
                </div>
              )}
              {c.loyaltyTier === "VIP" && (
                <div className="absolute top-0 right-0 w-16 h-16 overflow-hidden">
                  <div className="absolute top-4 -right-4 w-24 bg-purple-500 text-foreground text-[10px] font-bold text-center py-1 rotate-45 transform origin-center shadow-lg">
                    VIP
                  </div>
                </div>
              )}
              ...
              <div className="flex items-start gap-4 mb-6">
                <div className="w-14 h-14 rounded-full bg-onyx-elevated border border-border overflow-hidden flex items-center justify-center">
                  <img src={`https://ui-avatars.com/api/?name=${c.name}&background=1a1a1a&color=D4A843`} alt={c.name} />
                </div>
                <div className="flex-1 min-w-0 pr-8">
                  <h4 className="text-[18px] font-serif text-foreground truncate">{c.name}</h4>
              ...
              <div className="space-y-4 flex-1">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[15px] font-medium text-foreground">₹{c.lifetimeValue.toLocaleString("en-IN")}</span>
                </div>
                <div className="flex justify-between items-center">
              ...
              <div className="grid grid-cols-3 gap-3 mt-6 pt-5 border-t border-[#222]">
                <button className="flex flex-col items-center justify-center gap-1.5 h-14 bg-onyx-elevated rounded-lg border border-border hover:bg-secondary transition-colors group">
                  <User className="w-4 h-4 text-[#888] group-hover:text-foreground" />
                  <span className="text-[10px] text-[#888] uppercase tracking-wide group-hover:text-foreground">Profile</span>
                </button>
                <button className="flex flex-col items-center justify-center gap-1.5 h-14 bg-onyx-elevated rounded-lg border border-border hover:bg-secondary transition-colors group">
                  <MessageSquare className="w-4 h-4 text-[#888] group-hover:text-foreground" />
                  <span className="text-[10px] text-[#888] uppercase tracking-wide group-hover:text-foreground">Message</span>
                </button>
                <button className="flex flex-col items-center justify-center gap-1.5 h-14 bg-onyx-elevated rounded-lg border border-[#D4A843]/30 hover:bg-[#D4A843]/10 transition-colors group">
        ...
        <div className="flex items-center justify-between pb-10">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={page === 1}
              className="w-9 h-9 rounded-lg border border-[#222] bg-onyx-surface flex items-center justify-center text-[#888] hover:text-foreground disabled:opacity-50"
            >
                ...
                <button
                  ...
                  className={`w-9 h-9 rounded-lg border ${
                    page === i + 1 
                      ...
                      : "border-[#222] bg-onyx-surface text-[#888] hover:text-foreground"
                  }`}
                >
                  {i + 1}
                </button>
