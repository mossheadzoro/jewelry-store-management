...
export async function getDashboardData(branchId: number | null, dateRange: string) {
  ...
  const pAvailableItems = prisma.productItem.count({ where: { ...branchFilter, quantity: { gt: 0 } } });
  const pReservedItems = prisma.productItem.count({ where: { ...branchFilter, reservedQty: { gt: 0 } } });
  const pOutOfStock = prisma.productItem.count({ where: { ...branchFilter, quantity: 0 } });
  const pLowStock = prisma.productItem.count({ where: { ...branchFilter, quantity: { gt: 0, lte: 2 } } });
  ...
  const pNewCustomers = prisma.customer.count({ where: { createdAt: { gte: startDate, lte: endDate } } });
  const pVipCustomers = prisma.customerTag.count({ 
    where: { tagDefinition: { name: { contains: 'VIP', mode: 'insensitive' } } } 
  });
  const pPendingOrders = prisma.order.count({ where: { ...branchFilter, status: { in: ['CREATED', 'ASSIGNED', 'IN_PROGRESS'] } } });
  const pUrgentOrders = prisma.order.count({ where: { ...branchFilter, priority: { in: ['URGENT', 'RUSH'] }, status: { notIn: ['DELIVERED', 'CANCELLED', 'RETURNED'] } } });
  const pOverdueOrders = prisma.order.count({ where: { ...branchFilter, deliveryDate: { lt: new Date() }, status: { notIn: ['DELIVERED', 'CANCELLED', 'RETURNED'] } } });
  const pTodayDeliveries = prisma.order.count({ where: { ...branchFilter, deliveryDate: { gte: startOfDay(new Date()), lte: endOfDay(new Date()) } } });
  ...
  const [
    currentSales, prevSales, currentOrders, prevOrders, currentInvoiceItems,
    availableItems, reservedItems, outOfStock, lowStock, transfersPending,
    newCustomers, vipCustomers,
    pendingOrders, urgentOrders, overdueOrders, todayDeliveries, karigarBusy,
    payments, outstandingInvoices, advances,
    invoiceItems
  ] = await Promise.all([
    pCurrentSales, pPrevSales, pCurrentOrders, pPrevOrders, pCurrentInvoiceItems,
    pAvailableItems, pReservedItems, pOutOfStock, pLowStock, pTransfersPending,
    pNewCustomers, pVipCustomers,
    pPendingOrders, pUrgentOrders, pOverdueOrders, pTodayDeliveries, pKarigarBusy,
    pPayments, pOutstandingInvoices, pAdvances,
    pInvoiceItemsList
  ]);
  ...
  if (!branchId) {
    ...
    branchComparison = await Promise.all(branches.map(async (b) => {
      const [sales, orders, prevSalesBranch] = await Promise.all([
      ...
      return { id: b.id, name: b.name, sales: curr, orders, customers: sales._count._all, profit: curr * 0.16, growth };
    }));
  }
  ...
  const chartDataPromises = daysArray.map(async (i) => {
    // Chronological order: from (numDays - 1) days ago to today
