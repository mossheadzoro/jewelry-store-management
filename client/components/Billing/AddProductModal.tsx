...
import { Scale, BarChart2, Image as ImageIcon, RefreshCcw } from "lucide-react";
...
interface Props {
  ...
  onRefreshMetalRate?: () => Promise<void>;
}
...
export default function AddProductModal({
  ...
  // Update local state when product changes to ensure pre-existing other charges and initial values are pre-filled
  useEffect(() => {
    if (product) {
      setMaking(product.makingChargePercent ?? product.makingPercent ?? 0);
      setDiscount(product.discountOnMaking ?? 0);
      // Pre-fill pre-existing other charges if present on product item (otherChargesPrice, additionalCharge, stoneCharge, etc.)
      const preExistingOtherCharges = product.otherChargesPrice ?? product.additionalCharge ?? product.otherChargeVal ?? product.stoneCharge ?? 0;
      setExtra(typeof preExistingOtherCharges === 'number' ? preExistingOtherCharges : parseFloat(preExistingOtherCharges) || 0);
    }
  }, [product]);
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="bg-[#0f0f0f] border-[#222] text-foreground w-[95vw] max-w-4xl sm:max-w-4xl p-0 overflow-hidden shadow-2xl [&>button]:hidden">
        ...
        <div className="p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1fr_260px] gap-8">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input 
                    disabled 
                    value={product.name} 
                    className="bg-onyx-elevated border-border text-foreground disabled:opacity-70 h-11" 
                  />
                </div>
                <div>
                ...
                <div>
                  ...
                  <Input
                    disabled
                    value={
                      [product?.subCategory?.category?.name || product?.category?.name, product?.subCategory?.name]
                        .filter(Boolean)
                        .join(" - ") || "Uncategorized"
                    }
                    className="bg-onyx-elevated border-border text-foreground disabled:opacity-70 h-11"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-onyx-elevated border border-border rounded-xl p-5 relative overflow-hidden">
                  <div className="relative z-10">
                    <p className="text-xs text-[#888] font-medium mb-1">Gross Weight</p>
                    <div className="flex items-baseline gap-2">
                      <span className="text-3xl font-light text-foreground">{product.gsWeight?.toFixed(3) || "0.000"}</span>
                ...
                <div className="bg-onyx-elevated border border-border rounded-xl p-5 relative overflow-hidden">
                  <div className="relative z-10">
                    ...
                    <div className="flex items-baseline gap-2">
                      <span className="text-3xl font-light text-foreground">{product.ntWeight?.toFixed(3) || "0.000"}</span>
                      <span className="text-sm text-[#777]">gms</span>
                    </div>
                  </div>
                  <div className="absolute right-4 bottom-4 w-10 h-10 rounded-full bg-secondary flex items-center justify-center border border-border">
              ...
              {/* PRICING STRUCTURE */}
              <SectionHeading title="Making & Additional Charges" />
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-onyx-elevated border border-border rounded-xl p-5">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Input
                      inputMode="decimal"
                      pattern="[0-9]*"
                      className="no-spinner border-none bg-transparent text-xl text-foreground p-0 h-auto focus-visible:ring-0"
                      value={metalRate}
                    ...
                    {onRefreshMetalRate && (
                      <button
                        onClick={async () => {
                          if (onRefreshMetalRate) {
                            await onRefreshMetalRate();
                          }
                        }}
                        className="p-1.5 hover:bg-[#2a2a2a] rounded-md transition-colors group ml-auto"
                        aria-label="Refresh metal rate"
                        title="Refresh live rate"
                      >
                        <RefreshCcw className="w-4 h-4 text-[#888] group-hover:text-[#d4a843]" />
                      </button>
                    )}
                  </div>
                  <div className="mt-4 pt-4 border-t border-border">
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span className="text-foreground">₹{metalValue.toFixed(2)}</span>
                    </div>
                  </div>
                </div>

                <div className="bg-onyx-elevated border border-border rounded-xl p-5">
                  <p className="text-[10px] font-bold text-[#888] tracking-widest uppercase mb-3">Charges Breakdown</p>
                  ...
                  <div className="mt-3">
                    <label className="text-xs text-[#666] mb-1 block">Additional Charges (₹)</label>
                    <Input
                      inputMode="decimal"
                      pattern="[0-9]*"
                      className="bg-secondary border-border text-foreground h-8 text-sm"
            ...
            <div className="flex flex-col">
              ...
              <div className="w-full aspect-square bg-onyx-elevated border border-dashed border-border rounded-xl flex items-center justify-center relative overflow-hidden mb-6">
                {product.image ? (
                  <img
                    src={product.image}
                    alt={product.name}
                ...
                <div className="bg-onyx-elevated border border-border rounded-xl p-5">
                  <div className="space-y-2 mb-4">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-foreground">₹{makingValue.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                    ...
                    <div className="flex justify-between text-sm">
                      <span className="text-[#888]">Additional Charges</span>
                      <span className="text-foreground">₹{extra.toFixed(2)}</span>
                    </div>
                  </div>
        ...
        {/* FOOTER ACTIONS */}
        <div className="bg-onyx border-t border-[#222] p-6 flex items-center justify-between">
          <button 
            onClick={() => {
              setMetalRate(billingMetalRate);
            ...
            className="text-xs font-bold text-[#777] uppercase tracking-wider hover:text-foreground transition-colors flex items-center gap-2"
          >
            ...
            <button
              onClick={() => {
                ...
                onConfirm({
                  ...
                  otherChargesPrice: extra,
                  finalTotal: total,
                });
                onClose();
              }}
              className="px-8 py-3 bg-[#d4a843] text-foreground hover:bg-[#b58b2e] text-xs font-bold tracking-widest uppercase rounded-full transition-colors"
            >
              Add to Invoice
            </button>
          </div>
