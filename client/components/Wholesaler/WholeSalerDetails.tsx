...
export default function WholeSalerDetails({ wholesaler: initialData }: Props) {
  ...
  if (!data) return <div className="min-h-screen bg-[#060b13] p-8 text-white">Loading...</div>;

  const totalOutstandingBalance = data.moneyBal;
  const activeOrdersCount = data.orders?.filter(o => !["DELIVERED", "CANCELLED", "RETURNED"].includes(o.status)).length || 0;
  ...
  return (
    <div className="min-h-screen bg-[#0a101b] text-white p-8 w-full font-sans">
      ...
      <div className="bg-[#111827] rounded-3xl p-8 mb-6 border border-gray-800 shadow-xl relative overflow-hidden">
        ...
        <div className="flex justify-between items-start">
          <div className="flex gap-6 items-center z-10">
            ...
            <div className="flex flex-col gap-1">
              <h1 className="text-3xl font-bold tracking-tight text-white mb-1">{data.name}</h1>
          ...
          <button className="flex items-center gap-2 bg-transparent text-gray-300 hover:text-white border border-gray-600 hover:border-gray-400 px-5 py-2.5 rounded-full text-sm font-medium transition-colors z-10">
            <Edit2 size={16} /> Edit Profile
          </button>
        </div>

        {/* Stats Row within the card */}
        <div className="grid grid-cols-3 mt-10 pt-8 border-t border-gray-800/60 z-10 relative">
          <div className="flex flex-col gap-1.5">
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-widest">Outstanding Balance</span>
            <span className="text-2xl font-bold text-white">₹{Math.abs(totalOutstandingBalance).toLocaleString("en-IN")}</span>
          </div>
          <div className="flex flex-col gap-1.5 border-l border-gray-800/60 pl-8">
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-widest">Active Orders</span>
            <span className="text-2xl font-bold text-blue-400">{activeOrdersCount} Orders</span>
          </div>
          <div className="flex flex-col gap-1.5 border-l border-gray-800/60 pl-8">
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-widest">Last Order</span>
            <span className="text-2xl font-bold text-white">{lastOrderDate}</span>
          </div>
        </div>
      </div>

      {/* Tabs */}
      <div className="flex gap-8 border-b border-gray-800 mb-8 overflow-x-auto scollbar-hide">
        {TABS.map((tab) => (
          <button
            key={tab}
          ...
          >
            ...
            {tab === "Jewellery Orders" && data.orders?.length ? (
              <span className={`ml-2 px-2 py-0.5 rounded-full text-xs bg-[#1a2332] border border-gray-800 ${activeTab === tab ? "text-blue-400" : "text-gray-500"}`}>
                {data.orders.length}
              </span>
            ) : null}
            {activeTab === tab && (
      ...
      <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
        ...
        {(activeTab === "Invoices" || activeTab === "Design Library" || activeTab === "Notes") && (
          <div className="flex flex-col items-center justify-center p-20 bg-[#111827] rounded-2xl border border-gray-800 text-gray-500">
