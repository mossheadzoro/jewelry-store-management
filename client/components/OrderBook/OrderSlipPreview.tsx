    ...
    w.document.write(`<html><head><title>Order Slip - ${order.orderNumber}</title>
      <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .info-box { background:#f5f5f5; border:1px solid #e0e0e0; border-radius:8px; padding:10px 14px; }
        ...
        .terms h4 { font-size:10px; font-weight:700; text-transform:uppercase; letter-spacing:2px; margin-bottom:8px; }
        .terms li { font-size:10px; color:#888; margin:4px 0; }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <button onClick={onClose} className="flex items-center gap-2 text-[13px] text-[#888] hover:text-white transition-colors mb-3">
            <ArrowLeft className="w-4 h-4" /> Back to Orders
          </button>
          <h1 className="text-[28px] font-bold text-white">Order Preview</h1>
      ...
      {/* Slip Preview */}
      <div className="max-w-[650px] mx-auto bg-[#141414] border border-[#222] rounded-2xl p-8" ref={slipRef}>
        <div className="slip">
          {/* Header */}
          <div className="header" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "2px solid #D4A843", paddingBottom: 16, marginBottom: 24 }}>
            <div>
              <div className="store-name" style={{ fontSize: 20, fontWeight: 800, letterSpacing: 2, color: "#D4A843" }}>THE CURATED ATELIER</div>
              <div className="store-sub" style={{ fontSize: 10, color: "#888", letterSpacing: 1, marginTop: 4 }}>HERITAGE WING, NEW DELHI BRANCH</div>
              <div className="store-sub" style={{ fontSize: 10, color: "#888", letterSpacing: 1 }}>GSTIN: 07AAAAA0000A1Z5</div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 9, color: "#888", textTransform: "uppercase", letterSpacing: 1 }}>Date of Issue</div>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 700, color: "#fff" }}>{order.customerName}</div>
            <div style={{ fontSize: 12, color: "#888", marginTop: 2 }}>+91 {order.customerMobile}</div>
          </div>

          {/* Expected Delivery & Reference */}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
            <div style={{ background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: 10, padding: "10px 16px" }}>
              <div style={{ fontSize: 9, color: "#888", textTransform: "uppercase", letterSpacing: 1 }}>Expected Delivery</div>
            ...
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 9, color: "#888", textTransform: "uppercase", letterSpacing: 1 }}>Order Reference</div>
              <div style={{ fontSize: 16, fontWeight: 700, color: "#fff" }}>#{order.orderNumber}</div>
              <div style={{ fontSize: 9, color: "#888", textTransform: "uppercase", letterSpacing: 1, marginTop: 8 }}>Slip ID</div>
              <div style={{ fontSize: 16, fontWeight: 800, color: "#D4A843", fontStyle: "italic" }}>{order.advance?.advanceReceiptNumber || "—"}</div>
            </div>
          </div>

          {/* Commissioned Items */}
          <div style={{ fontSize: 10, fontWeight: 700, color: "#D4A843", textTransform: "uppercase", letterSpacing: 2, marginBottom: 12, borderBottom: "1px solid #2a2a2a", paddingBottom: 8 }}>
            Commissioned Items
          </div>
          <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 20 }}>
            <thead>
              <tr>
                <th style={{ fontSize: 9, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: 1, textAlign: "left", padding: "8px 12px", borderBottom: "1px solid #2a2a2a" }}>Category</th>
                <th style={{ fontSize: 9, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: 1, textAlign: "left", padding: "8px 12px", borderBottom: "1px solid #2a2a2a" }}>Description</th>
                <th style={{ fontSize: 9, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: 1, textAlign: "right", padding: "8px 12px", borderBottom: "1px solid #2a2a2a" }}>Weight / Size</th>
              </tr>
            </thead>
            <tbody>
              {order.items?.map((item, idx) => (
                <tr key={idx}>
                  <td style={{ fontSize: 13, fontWeight: 600, color: "#fff", padding: "10px 12px", borderBottom: "1px solid #1a1a1a" }}>{item.category?.name || "—"}</td>
                  <td style={{ fontSize: 12, color: "#ccc", padding: "10px 12px", borderBottom: "1px solid #1a1a1a" }}>
                    ...
                    {item.images && item.images.length > 0 && (
                      <div style={{ display: "flex", gap: "4px", marginTop: "8px", flexWrap: "wrap" }}>
                        {item.images.map((img, i) => (
                          <img key={i} src={img} style={{ width: "40px", height: "40px", objectFit: "cover", borderRadius: "4px", border: "1px solid #333" }} />
                        ))}
                      </div>
                    )}
                  </td>
                  <td style={{ fontSize: 13, fontWeight: 700, color: "#D4A843", padding: "10px 12px", borderBottom: "1px solid #1a1a1a", textAlign: "right" }}>
          ...
          {/* Advance Summary */}
          <div style={{ background: "#1a1a1a", border: "1px solid #D4A843", borderRadius: 12, padding: 16, marginBottom: 20 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 8 }}>
              <span style={{ color: "#888" }}>Cash Deposit</span>
              <span style={{ color: "#fff", fontWeight: 600 }}>₹ {Number(order.advance?.moneyAmount || 0).toLocaleString("en-IN")}</span>
            </div>
            {Number(order.advance?.metalWeight || 0) > 0 && (
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 8 }}>
                <span style={{ color: "#888" }}>Exchange Metal ({order.advance?.metalPurity || "22K"})</span>
                <span style={{ color: "#fff", fontWeight: 600 }}>{Number(order.advance?.metalWeight || 0).toFixed(3)} g</span>
              </div>
            )}
          </div>
          ...
          <div style={{ borderTop: "1px solid #2a2a2a", paddingTop: 16, marginTop: 16 }}>
            <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: 2, color: "#fff", marginBottom: 8 }}>Terms & Conditions</div>
            <ul style={{ listStyle: "none", padding: 0 }}>
              {["Orders once placed cannot be cancelled after 24 hours of confirmation.",
                ...
                <li key={i} style={{ fontSize: 10, color: "#666", marginBottom: 4 }}>• {t}</li>
              ))}
            </ul>
          </div>

          {/* Footer */}
          <div style={{ textAlign: "center", marginTop: 24, fontSize: 9, color: "#555", letterSpacing: 1 }}>
