      ...
      <div className="w-full max-w-[550px] bg-[#111113] border-l border-[#1F1F24] h-full flex flex-col justify-between shadow-2xl relative animate-in slide-in-from-right duration-300">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          ...
          {data && (
            <div className="p-6 space-y-6">
                  ...
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-[#1F1F24] text-[#F0EBE0]">
                      {data.products.map((item: any, idx: number) => {
                        const showHuidWarning = !item.barcode; // or if item misses huid
                        const karatage = getKaratageLabel(item.purity);
                        return (
                          <tr key={idx} className="hover:bg-[#222228] transition-colors">
                            <td className="px-4 py-3">
                              <p className="font-semibold">{item.name}</p>
                              <span className="text-[10px] text-[#6B6560] font-mono">{item.productCode} • {karatage}</span>
                            </td>
                            ...
                            <td className="px-4 py-3 text-right font-mono font-semibold">{formatINR(item.metalRate * item.ntWeight + (item.additionalCharge || 0))}</td>
                          </tr>
                        );
                      })}
                    </tbody>
              ...
              {/* Calculation Summary / Charges */}
              <div className="space-y-2 border-t border-[#1F1F24] pt-4 text-xs">
                ...
                {data.hallmarkCharge && (
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold font-mono text-[#F0EBE0]">Included</span>
                  </div>
                )}
                {/* GST */}
                <div className="flex justify-between pt-2 border-t border-[#1F1F24] border-dashed">
                  <span className="text-[#6B6560]">CGST (1.5%)</span>
                  <span className="font-semibold font-mono text-[#F0EBE0]">{formatINR(data.products.reduce((s: number, p: any) => s + (p.cgst || 0), 0) || (data.grandTotal * 0.015))}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#6B6560]">SGST (1.5%)</span>
                  <span className="font-semibold font-mono text-[#F0EBE0]">{formatINR(data.products.reduce((s: number, p: any) => s + (p.sgst || 0), 0) || (data.grandTotal * 0.015))}</span>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="p-6 border-t border-[#1F1F24] bg-[#0A0A0B] flex flex-col gap-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-2xl font-bold font-mono text-[#C9943A]">{data ? formatINR(data.grandTotal || 0) : "₹0"}</span>
          </div>
          {data && (
            <div className="grid grid-cols-2 gap-3 pt-2 border-t border-[#1F1F24] border-dashed text-xs">
              <div className="rounded-lg bg-green-500/5 border border-green-500/10 p-2 flex justify-between items-center">
                ...
                <span className="font-bold font-mono text-green-400">{formatINR(data.payments.reduce((s: number, p: any) => s + (parseFloat(p.amount) || 0), 0))}</span>
              </div>
              <div className="rounded-lg bg-amber-500/5 border border-amber-500/10 p-2 flex justify-between items-center">
                ...
                <span className="font-bold font-mono text-amber-400">{formatINR(Math.max(0, data.grandTotal - data.payments.reduce((s: number, p: any) => s + (parseFloat(p.amount) || 0), 0)))}</span>
              </div>
            </div>
          )}
