...
export default function EditProductPage({ id }: EditProductPageProps) {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        setLoading(true);
        const [productRes, subCategoryRes, settingsRes] = await Promise.all([
          axios.get(`/api/inventory/product/fetchById/${id}`),
          axios.get(`/api/inventory/subcategory/fetchAll?branchId=${selectedBranch.id}`),
          axios.get(`/api/settings/product?branchId=${selectedBranch.id}`),
        ]);

        const product = productRes.data;
        const subCats = subCategoryRes.data;

        if (settingsRes.data?.mediaConfig) {
          setGlobalMediaConfig(settingsRes.data.mediaConfig);
        }
  ...
  if (loading && form.name === "") {
    return (
      <div className="min-h-screen bg-onyx flex flex-col justify-center items-center gap-4 text-foreground">
        <div className="relative flex items-center justify-center">
          <div className="w-16 h-16 border-4 border-[#d4a843] border-t-transparent rounded-full animate-spin"></div>
        </div>
        <span className="text-muted-foreground text-sm tracking-widest uppercase font-semibold">Loading product details...</span>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-onyx text-foreground p-8 w-full">
      ...
      <div className="max-w-6xl mx-auto mb-8 flex items-center justify-between">
        ...
        <h1 className="text-3xl font-light text-foreground/80 tracking-tight flex items-center gap-3">
          Edit Product: <span className="text-[#d4a843] font-normal">{form.name || "Loading..."}</span>
        </h1>
      </div>

      <div className="max-w-6xl mx-auto bg-[#0f0f0f] border border-border rounded-2xl p-8 relative">
        {loading && (
          <div className="absolute inset-0 bg-[#0f0f0f]/80 backdrop-blur-sm z-50 rounded-2xl flex flex-col justify-center items-center gap-4">
            ...
            <span className="text-muted-foreground text-xs tracking-wider uppercase">Saving Changes...</span>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8">
          ...
          <div className="space-y-8">
            ...
            <div>
              <div className="flex items-center gap-4 mb-6">
                ...
                <div className="h-[1px] flex-1 bg-secondary/60"></div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  ...
                  <Input
                    ...
                    className="bg-onyx-elevated border-border text-foreground h-11 focus-visible:ring-[#d4a843]"
                  />
                </div>
                <div>
                  <Label className="text-xs text-muted-foreground font-medium mb-1.5 block">Product Code</Label>
                  <Input
                    name="productCode"
                    value={form.productCode}
                    onChange={handleChange}
                    disabled={!(user?.systemRole === "ADMIN" || user?.role === "ADMIN" || user?.systemRole === "MANAGER" || user?.role === "MANAGER")}
                    className="bg-onyx-elevated border-border text-foreground h-11 disabled:opacity-50"
                  />
                </div>
                <div>
                  <Label className="text-xs text-muted-foreground font-medium mb-1.5 block">Barcode</Label>
                  <Input
                    name="barcode"
                    value={form.barcode}
                    onChange={handleChange}
                ...
                <div>
                  <Label className="text-xs text-muted-foreground font-medium mb-1.5 block">HUID Number</Label>
                  <Input
                    name="huidNumber"
                ...
                <div>
                  <Label className="text-xs text-muted-foreground font-medium mb-1.5 block">Purity</Label>
                  {isSilver ? (
                    <Input
                      name="purity"
                      value={form.purity}
                ...
                <div>
                  <Label className="text-xs text-muted-foreground font-medium mb-1.5 block">Size</Label>
                  <Input
                    name="size"
                    value={form.size}
                    onChange={handleChange}
                ...
                <div>
                  <Label className="text-xs text-muted-foreground font-medium mb-1.5 block">SubCategory</Label>
                  <select
                    name="subCategoryId"
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-onyx-elevated border border-border rounded-xl p-5 relative overflow-hidden focus-within:border-[#d4a843] transition-colors">
                  <p className="text-xs text-muted-foreground font-medium mb-2">Gross Weight</p>
                  <div className="flex items-baseline gap-2">
                    <Input
                      ...
                      onChange={handleChange}
                      placeholder="00.000"
                      className="border-none bg-transparent text-3xl font-light text-foreground p-0 h-auto focus-visible:ring-0 w-32"
                    />
                    <span className="text-sm text-muted-foreground">gms</span>
                  </div>
                </div>

                <div className="bg-onyx-elevated border border-border rounded-xl p-5 relative overflow-hidden focus-within:border-[#d4a843] transition-colors">
                  <p className="text-xs text-muted-foreground font-medium mb-2">Net Weight <span className="text-[10px] text-gray-600">(auto-calculated)</span></p>
                  <div className="flex items-baseline gap-2">
                    <Input
                      name="ntWeight"
                      value={form.ntWeight}
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 bg-onyx-elevated border border-border rounded-md px-3 h-11">
                    ...
                    <Input
                      ...
                      onChange={handleChange}
                      placeholder="0"
                      className="border-none bg-transparent text-foreground p-0 h-full focus-visible:ring-0 text-sm w-full"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-onyx-elevated border border-border rounded-xl p-5">
                <div className="flex items-center gap-2">
                  ...
                  <Input
                    ...
                    onChange={handleChange}
                    placeholder="0.00"
                    className="border-none bg-transparent text-2xl text-foreground p-0 h-auto focus-visible:ring-0"
                  />
                </div>
              </div>
            </div>
                ...
                <div className="space-y-4">
                  <div className="flex justify-end">
                    <Button
                      ...
                      className="bg-onyx-elevated hover:bg-secondary text-[#d4a843] border border-border h-9 text-xs flex items-center gap-1.5 rounded-xl"
                    >
                  ...
                  {form.stoneDetails.map((stone, index) => (
                    <div key={index} className="bg-onyx-elevated border border-border rounded-xl p-5 relative">
                      <button
                        type="button"
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                        <div>
                          ...
                          <Input
                            ...
                            className="bg-secondary border-border text-foreground h-9 text-xs mt-1"
                          />
                        </div>
                        <div>
                          <Label className="text-[10px] text-muted-foreground uppercase">Carat</Label>
                          <Input
                            value={stone.carat || ''}
                        ...
                        <div>
                          <Label className="text-[10px] text-muted-foreground uppercase">Weight (g)</Label>
                          <Input
                            type="number"
                            value={stone.weight || ''}
          ...
          <div className="flex flex-col gap-6">
            <div>
              ...
              <div className="bg-onyx-elevated border border-dashed border-border rounded-xl p-4 flex flex-col items-center justify-center gap-4">
                {form.image && (
                  <div className="relative w-full aspect-square max-h-48 rounded-xl overflow-hidden bg-background/40 border border-border/50 flex items-center justify-center">
                    <img
                      src={form.image}
                ...
                <UploadImage onUpload={(url) => setForm((prev) => ({ ...prev, image: url }))} autoCompress={globalMediaConfig?.autoCompress} />
              </div>
            </div>
            ...
            <div>
              ...
              <Textarea
                ...
                className="bg-onyx-elevated border-border text-foreground resize-none h-44"
              />
            </div>
          </div>
        </div>
        ...
        {/* Footer Actions */}
        <div className="bg-onyx border-t border-border/60 p-6 flex items-center justify-end gap-3 mt-8 -mx-8 -mb-8 rounded-b-2xl">
          <Button
            type="button"
            variant="ghost"
            onClick={() => router.back()}
            className="text-xs font-bold text-muted-foreground uppercase tracking-wider hover:text-foreground transition-colors h-11 px-6 rounded-full"
          >
            Cancel
          </Button>
          <Button
            ...
            disabled={loading}
            className="px-6 h-11 bg-[#d4a843] text-foreground hover:bg-[#b58b2e] text-xs font-bold tracking-widest uppercase rounded-full transition-colors flex items-center gap-2"
          >
