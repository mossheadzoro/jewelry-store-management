...
export default function ReceiveStampingForm({
  open,
  setOpen,
  branches,
  onSuccess,
  ...
  const [showStoneDetails, setShowStoneDetails] = useState(
    stampingProduct?.stoneDetails && stampingProduct.stoneDetails.length > 0
  );
  const [stones, setStones] = useState<RegisterStone[]>(
    stampingProduct?.stoneDetails || []
  );
  ...
  const {
    ...
    defaultValues: {
      ...
      image: stampingProduct?.image || "",
      description: stampingProduct?.description || "",
    },
  });
  ...
  const selectedSubCategory = subCategories.find(
    (sc) => sc.id === parseInt(watchedSubCategoryId?.toString() || "0"),
  );
    ...
    const fetchBranchProductSettings = async () => {
      ...
      try {
        const res = await axios.get(
          `/api/settings/product?branchId=${selectedBranch.id}`,
        );
  ...
  const availablePurities = useMemo(() => {
    if (branchMetalSettings.length > 0) {
      const targetMetalName = isSilver
        ? "silver"
        : categoryName.toLowerCase().includes("platinum")
          ? "platinum"
          : "gold";
      const foundMetal = branchMetalSettings.find(
        (m: any) =>
          m.name?.toLowerCase().includes(targetMetalName) && m.active !== false,
      );

      if (
        foundMetal &&
        Array.isArray(foundMetal.purities) &&
        foundMetal.purities.length > 0
      ) {
        return foundMetal.purities
          .map((p: any) => {
            const caratStr = typeof p === "string" ? p : p.carat || "";
            const rawNum = caratStr.replace(/[^0-9]/g, "");
            return {
              label:
                caratStr.endsWith("K") || caratStr.length >= 3
                  ? caratStr
                  : `${caratStr}K`,
    ...
    const fetchSubCategories = async () => {
      try {
        ...
        let fetchedData = res.data.filter((s: any) => {
           ...
           return !["UNMARKED JEWELLERY", "STAMPING CENTER", "RAW FINE GOLD", "RAW FINE SILVER", "RAW METAL"].includes(catName);
        });
  ...
  useEffect(() => {
    if (
      ...
      watchedSubCategoryId !== undefined
    ) {
      generateCodes();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const handleReceiveSubmit = async (data: RegisterForm) => {
    try {
      ...
      const finalSubCategoryId = isNaN(subCategoryIdAsNumber)
        ? 0
        : subCategoryIdAsNumber;

      const foundSubCat = subCategories.find(
        (sc: any) => sc.id === finalSubCategoryId,
      );
      ...
      if (!showBasePrice) {
        ...
        if (data.otherChargesPrice) {
          finalOtherChargesPrice = parseFloat(
            data.otherChargesPrice.toString(),
          );
        }
      }
  ...
  const handleAddStone = () => {
    setStones([
      ...stones,
      { carat: "", name: "", quality: "", price: "", quantity: 1, weight: 0 },
    ]);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="bg-[#0f0f0f] border-[#222] text-foreground w-full sm:max-w-5xl lg:max-w-6xl p-0 overflow-hidden shadow-2xl [&>button]:hidden h-[90vh] flex flex-col">
            ...
            <DialogHeader>
              <div className="flex items-center gap-3">
                ...
                <span className="px-3 py-1 rounded-full border border-[#d4a843]/30 bg-[#d4a843]/10 text-[10px] font-bold text-[#d4a843] tracking-widest uppercase">
                  From Stamping Center
                </span>
              </div>
              <DialogDescription className="text-sm text-[#777] mt-1 text-left">
                Finalize the stamped product and move it to your main inventory.
              </DialogDescription>
            </DialogHeader>
            <button
          ...
          {Object.keys(errors).length > 0 && (
            <div className="mb-6 p-4 bg-red-950/40 border border-red-500/40 rounded-xl text-red-200 text-xs">
              <p className="font-bold mb-2">
                Please fix the following validation errors to Add to Queue:
              </p>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="col-span-2">
                      <Label className="text-xs text-[#888] font-medium mb-1.5 block">
                        Name
                      </Label>
                      <Input
                        {...register("name")}
                        placeholder="e.g. Maharani Polki Choker"
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        className="bg-onyx-elevated border-border text-foreground h-11 opacity-70"
                      />
                    </div>
                    <div>
                      <Label className="text-xs text-[#888] font-medium mb-1.5 block">
                    ...
                    <div>
                      ...
                      {errors.purity && (
                        <p className="text-red-500 text-xs mt-1">
                          Please select a valid purity configured for{" "}
                    ...
                    <div className="col-span-2 grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <select
                          ...
                          className="w-full bg-onyx-elevated border border-border text-foreground rounded-md px-3 h-11 focus:border-[#d4a843] outline-none"
                        >
                          {categories.length === 0 ? (
                            <option value="">Please Create Category or SubCategory first</option>
                          ) : (
                      ...
                      <div>
                        ...
                        >
                          {categories.length === 0 ? (
                            ...
                            subCategories
                              .filter((s: any) => s.category?.id === selectedCategoryId)
                              .map((s) => (
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-onyx-elevated border border-border rounded-xl p-5 relative overflow-hidden group focus-within:border-[#d4a843] transition-colors">
                      ...
                      {errors.gsWeight && (
                        <p className="text-red-500 text-xs mt-1 absolute bottom-2 left-5">
                          {errors.gsWeight.message}
                        </p>
                      )}
                    </div>
                  ...
                  <div>
                    <div className="flex items-center gap-4 mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Label
                          ...
                          className="text-[10px] font-bold uppercase tracking-widest text-[#d4a843] cursor-pointer"
                        >
                    ...
                    {showMakingCharge && (
                      <div className="bg-onyx-elevated border border-border rounded-xl p-5">
                        <p className="text-[10px] font-bold text-[#888] tracking-widest uppercase mb-3">
                          Making Charge
                        </p>
                        <div className="flex gap-3">
                          <div className="flex items-center gap-2 flex-1 bg-secondary border border-border rounded-md px-3">
                            ...
                            <Input
                              ...
                              placeholder="0.00"
                              className="border-none bg-transparent text-foreground p-0 h-11 focus-visible:ring-0 w-full"
                            />
                          </div>
                          <select
                            {...register("makingChargeType")}
                            className="bg-secondary border border-border text-foreground rounded-md h-11 px-3 text-sm focus:outline-none focus:border-[#d4a843] w-24"
                          >
                      ...
                      <div className="bg-onyx-elevated border border-border rounded-xl p-5">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Input
                            {...register("price")}
                            placeholder="0.00"
                            className="border-none bg-transparent text-xl text-foreground p-0 h-auto focus-visible:ring-0"
                          />
                        </div>
                      </div>
                    )}
                  ...
                  <div>
                    <div className="flex items-center gap-4 mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        <Checkbox
                          id="stoneDetails"
                          checked={showStoneDetails}
                          onCheckedChange={() =>
                            setShowStoneDetails(!showStoneDetails)
                          }
                          className="border-[#666] data-[state=checked]:bg-[#d4a843] data-[state=checked]:text-foreground"
                        />
                        <Label
                      ...
                      <div className="space-y-4">
                        <div className="flex justify-end">
                          <Button
                            ...
                            className="bg-secondary hover:bg-secondary text-[#d4a843] border border-border h-8 text-xs flex items-center gap-1"
                          >
                        ...
                        {stones.map((stone, index) => (
                          <div
                            key={index}
                            className="bg-onyx-elevated border border-border rounded-xl p-4 relative"
                          >
                            <button
                              type="button"
                              onClick={() => handleRemoveStone(index)}
                              className="absolute top-4 right-4 text-[#666] hover:text-red-400"
                            >
                              <X size={16} />
                            </button>
                            ...
                            <div className="grid grid-cols-3 gap-3">
                              <div>
                                <Label className="text-[10px] text-[#888] uppercase">
                                  Name
                                </Label>
                                <Input
                                  value={stone.name}
                                  ...
                                  className="bg-secondary border-border text-foreground h-8 text-xs mt-1"
                                />
                              </div>
                              <div>
                                <Label className="text-[10px] text-[#888] uppercase">
              ...
              <div className="flex flex-col gap-6">
                ...
                {isDiamond && (
                  <div>
                    ...
                    <div className="bg-onyx-elevated border border-dashed border-[#444] rounded-xl p-3">
                      <UploadImage
                        value={watch("certImage")}
                        onUpload={(url) => setValue("certImage", url)}
                      />
                    </div>
                  </div>
                )}
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    className="bg-onyx-elevated border-border text-foreground resize-none h-32"
                  />
                </div>
              </div>
            </div>
        ...
        {/* FOOTER ACTIONS */}
        <div className="bg-onyx border-t border-[#222] p-6 flex items-center justify-end shrink-0">
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={() => setOpen(false)}
            ...
            <button
              ...
              type="submit"
              disabled={loading}
              className="px-6 py-3 bg-[#d4a843] text-foreground hover:bg-[#b08b33] text-xs font-bold tracking-widest uppercase rounded-full transition-colors disabled:opacity-50"
            >
